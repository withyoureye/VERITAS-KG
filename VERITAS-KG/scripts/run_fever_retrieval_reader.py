...
def sentence_score(claim: str, sentence: str) -> float:
    c = set(tokenize(claim))
    s = set(tokenize(sentence))
    if not c:
        return 0.0
    overlap = len(c & s) / len(c)
    neg = any(tok in c for tok in {"not", "never", "no", "false", "fake"})
    return overlap + (0.08 if neg else 0.0)


def sentence_text(snippet: str) -> str:
    if ": " in snippet and "#" in snippet.split(": ", 1)[0]:
        return snippet.split(": ", 1)[1]
    return snippet


def classify(claim: str, evidence: str) -> str:
    if "No evidence available" in evidence:
        return "NOT ENOUGH INFO"
    evidence_text = sentence_text(evidence).lower()
    lower = claim.lower()
    if any(token in lower for token in [" not ", " never ", " no ", " false ", "disassociated", "refused"]):
        return "REFUTES"
    if any(token in evidence_text for token in ["not ", "never ", " no ", " however ", "although "]):
        claim_tokens = set(tokenize(claim))
        evidence_tokens = set(tokenize(evidence_text))
        if len(claim_tokens & evidence_tokens) >= 2:
            return "REFUTES"
    if sentence_score(claim, evidence_text) < 0.08:
        return "NOT ENOUGH INFO"
    return "SUPPORTS"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a retrieval+reader style FEVER baseline.")
    ...
    for candidates in load_groups(args.input, args.groups_limit):
        ...
        support_cands = [c for c in candidates if c.get("candidate_label") in {"SUPPORTS", "REFUTES"}]
        if not support_cands:
            support_cands = candidates
        retrieved = max(
            support_cands,
            key=lambda item: sentence_score(claim, sentence_text(str(item.get("evidence", {}).get("snippet", "")))),
        )
        predicted = classify(claim, str(retrieved.get("evidence", {}).get("snippet", "")))
        rows.append(
            {
                ...
                "retrieved_label": retrieved.get("candidate_label"),
    ...
    payload = {
        "baseline": "fever_pointer_retrieval_reader_proxy",
        "evaluated": len(rows),
        "accuracy": accuracy,
